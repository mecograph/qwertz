<template>
  <section class="space-y-4">
    ...
    <div class="grid gap-3 md:grid-cols-5">
      <div class="rounded-xl border border-slate-200 bg-white p-3" v-for="kpi in kpis" :key="kpi.label">
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <div class="mb-2 flex flex-wrap items-center gap-2">
        ...
        <select v-model="mode" class="rounded-md border border-slate-300 px-2 py-1 text-sm">
    ...
    <div class="grid gap-4 lg:grid-cols-3">
      <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm lg:col-span-2">
        <div class="mb-2 flex items-center justify-between">
          ...
          <button class="rounded-md border border-slate-300 px-3 py-1 text-sm" @click="openInData">Open in Data</button>
        </div>
      ...
      <aside class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div class="mt-3 grid gap-2">
          <div class="rounded bg-slate-50 p-2 text-sm">AmountAbs: {{ formatCurrency(detailKpis.amountAbs) }}</div>
          <div class="rounded bg-slate-50 p-2 text-sm">Transactions: {{ detailKpis.count }}</div>
          <div class="rounded bg-slate-50 p-2 text-sm">Avg: {{ formatCurrency(detailKpis.avg) }}</div>
          <div class="rounded bg-slate-50 p-2 text-sm">Largest: {{ formatCurrency(detailKpis.largest) }}</div>
        </div>
