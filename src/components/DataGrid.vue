<template>
  <section class="ui-card flex h-full min-h-0 flex-col p-4">
    ...
    <RecycleScroller class="mt-3 min-h-0 flex-1" :items="pagedRows" :item-size="44" key-field="id" v-slot="{ item }">
      <div class="grid grid-cols-8 gap-2 border-b px-2 py-2 text-sm">
        ...
        <div class="flex gap-1">
          <button class="ui-btn-ghost border px-2" @click="tx.duplicateRow(item.id)">Dup</button>
          <button class="ui-btn-ghost border px-2" @click="tx.deleteRow(item.id)">Del</button>
        </div>
      </div>
    </RecycleScroller>
    ...
    <div class="mt-3 flex flex-wrap items-center justify-between gap-2">
      <div class="flex gap-2">
        <button class="ui-btn" @click="tx.undo">Undo</button>
        <button class="ui-btn" @click="tx.redo">Redo</button>
      </div>

      <div class="flex items-center gap-2 text-sm">
        <button class="ui-btn disabled:opacity-50" :disabled="page === 1" @click="page--">Prev</button>
        <span>Page {{ page }} / {{ totalPages }}</span>
        <button class="ui-btn disabled:opacity-50" :disabled="page === totalPages" @click="page++">Next</button>
        <select class="rounded-md border border-slate-300 bg-white px-2 py-1.5" v-model.number="pageSize">
