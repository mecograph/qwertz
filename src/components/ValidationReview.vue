<template>
  <section class="term-pane">
    ...
    <p class="mt-1 text-sm">
      <span class="text-terminal-green">[OK]</span> Valid rows: {{ validCount }}
      <span class="mx-2 text-terminal-muted">|</span>
      <span :class="issues.length ? 'text-terminal-red' : 'text-terminal-green'">[{{ issues.length ? 'ERR' : 'OK' }}]</span> Invalid rows: {{ issues.length }}
    </p>
